    ...
    for dxcc, handler in dxcc_handlers.items():
        ...
        if items:
            ...
            if result is not None:
                handler_results[dxcc] = result

    if other_qsos:
        verified_other = process_qsos_other(other_qsos, qrz_api)
        if verified_other:
            handler_results['other'] = verified_other
...
Usage examples:
  ...
  %(prog)s -c config.json --from-date 2024-06-01
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to output ADIF file for QSLs to send'
    )
    ...
    try:
        ...
        try:
            ...
            all_qsl_qsos = []
            for key, qsos_list in handler_results.items():
                if qsos_list:
                    if key == 'other':
                        logger.info(f"Other (verified via QRZ): {len(qsos_list)} QSLs to send")
                    else:
                        logger.info(f"{get_dxcc_name(key)}: {len(qsos_list)} QSLs to send")
            ...
            logger.info(f"Total {len(all_qsl_qsos)} QSLs to send saved to: {args.output_adif}")
