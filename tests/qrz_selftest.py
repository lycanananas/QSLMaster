...
from qslmaster_cli.config import ConfigError, load_config, validate_config
from qslmaster_cli.qrz import QRZAPI, QRZAPIError
from qslmaster_version import get_user_agent


REQUIRED_CALLSIGNS = [
    'sq5fox', 'sp5pot', 'w1aw', 'k1n', 'w5xo', 'w0ai', 'n4oo', 'k6ller', 'vk9ns', 'zs6sn',
    'n1eoh', 'iz0tsc', 'ea4gcw', 'pw2d', 'cx9au', 'vk5zd', 'jt1co', 'g3v', 'yb1hr', 'f4enk',
    'iu2sln', 'ii2wog', 'om2vl', 'j51a', 'k3lr', 'w4an', 'ea3gke/qrp', 'wp4bri', 'yo2kar', 'g3zbu',
    'dr4w', 'g4ari', 'w4gm', 'g0mfr', 'w5rme', 'g3rkf', 'zw5b', 'g2f', '5x1j', 'vp5/n5zo',
    'vq9la', 'oh2bn', 'pi4cc', 'k5nd', 'oe6add', 'k7ra', 'df7gb', 'n8ii', 've1jf', 've3on',
]
...
def _fetch_random_callsigns(count: int) -> List[str]:
    ...
    def _print_progress(found: int, attempt: int) -> None:
        print(
            f'\rFetching random callsigns from QRZ: {found}/{count} (attempt {attempt}/{max_attempts})',
            end='',
            flush=True,
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.random is not None:
        ...
        if len(random_callsigns) < args.random:
            ...
            print(f'Fetched {len(random_callsigns)} random callsigns from QRZ')
